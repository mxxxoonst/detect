...
> 本文是**编码器(模块 A)观测输入构造层**的设计:容错解析器把**带噪样本**变成 `seg1(观测表面 + 试探归组) + seg2(RAW)`,喂 **encoder-only 抽取式**模型学**形式层 + 结构层**结构鲁棒表示。严格解析器只在预处理阶段取 tier1 干净种子,不在此。与现有 `grade_parse` / `partition_file` 的关系见正文。
...
seg1 的 `surface` **不是**抽象 IR `[(path, dtype)]`,而是把恢复出的结构**重渲染**成"保留语法骨架 **+ 保留真实值**"的规范串。三条规则:

1. **保留所有承载结构的标点与分隔符**:`{ } [ ] ( ) : , "` 及分隔符——**形式层噪声活在这里**,必须留给编码器看见。
2. **保留真实值,并行附类型标注**(**不**替换为类型槽):**合成语料的值本身不敏感、不违反 PII 红线**,且**值形(如 `alice@gmail.com` 的形状)是字段定型的关键信号**,抹成 `<str>` 反而把它丢掉。故值 token **原样保留**,另在 `struct_tags.type_tag` 上**并行**附 `⟨str⟩/⟨int⟩/⟨float⟩/⟨bool⟩/⟨null⟩`;超长自由文本值可截断控长。(红线约束的是**落盘持久化**,不是编码器瞬态消费;真实管线侧落盘另按 CLAUDE.md 脱敏。)
3. **归一非结构性表面噪声**:连续空白 → 单空格、去 BOM / 编码伪字符、记录内键序规范化。

示例:

表面**原样保留值**,类型以 `⟨⟩` 并行标注(下表 `⟨⟩` 挂在 `struct_tags.type_tag`,非替换值):

| 输入 | 规范化恢复表面(值原样 + 类型标注) | 说明 |
| --- | --- | --- |
| JSON `{"name":"Alice","age":30}` | `{"name":"Alice"⟨str⟩,"age":30⟨int⟩}` | 结构标点全保留,**值原样**,类型并行 |
| CSV 行 `Alice,30,NYC` | `Alice⟨str⟩,30⟨int⟩,NYC⟨str⟩` | 分隔符保留,值原样 |
| CSV 漂移行 `Ali,ce,30,NYC`(4 列 / 众数 3) | `Ali⟨str⟩,ce⟨str⟩,30⟨int⟩,NYC⟨str⟩` | **多出的逗号留着**,漂移可见 |
| SQL `INSERT INTO users VALUES (1,'ok')` | `INSERT INTO users VALUES (1⟨int⟩,'ok'⟨str⟩)` | 关键字/表名/括号保留,值原样 |

**对比**:当前 `SchemaUnit` 的 `[("name","str"),("age","int")]` 把括号 / 冒号 / 逗号**和值内容**全丢了——这正是规范化恢复表面要补回的信息(**值形对字段定型尤其关键**)。
...
- `type_tag`:仅 `value_slot` 才非空,`⟨str⟩/⟨int⟩/⟨float⟩/⟨bool⟩/⟨null⟩`(**值原样保留,类型并行标注**,不替换值)
...
### 对齐示例:`{"name":"Alice","age":30}`(值原样保留)

```text
idx  token    role           depth  field_path  type_tag  node_type
 0   {        open_bracket    0      -           -         object
 1   "name"   key             1      -           -         object
 2   :        kv_sep          1      -           -         object
 3   "Alice"  value_slot      1      name        ⟨str⟩     object
 4   ,        delimiter       1      -           -         object
 5   "age"    key             1      -           -         object
 6   :        kv_sep          1      -           -         object
 7   30       value_slot      1      age         ⟨int⟩     object
...
- 本设计服务于 **encoder-only 抽取式**的观测输入:`seg1(观测表面 + 试探归组) + seg2(RAW)`;不做重的双通道机制。
- seg1 采用**规范化恢复表面(保留语法 + 保留真实值)而非抽象 IR**,是为了回应"模板化掉括号 / 分隔符导致信息不足"的质疑。
- **结构理解正确性的验证**(boundary / re-anchor / field-binding 等结构探针指标)见 `encoder_pretraining_design.md` §6 / §九。
