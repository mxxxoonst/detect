...
def test_grade_json_tier1(samples_dir):
    path = os.path.join(samples_dir, "clean_users.json")
...
def test_grade_csv_tier1(samples_dir):
    path = os.path.join(samples_dir, "clean_users.csv")
...
def test_grade_sqlite_tier1(samples_dir):
    path = os.path.join(samples_dir, "clean_users.db")
...
def test_grade_free_text(samples_dir):
    path = os.path.join(samples_dir, "free_text_zh.txt")
...
def test_grade_csv_tier2_drift(samples_dir):
    path = os.path.join(samples_dir, "noisy_column_drift.csv")
    grade = grade_parse(path, "free_text", "utf-8")
